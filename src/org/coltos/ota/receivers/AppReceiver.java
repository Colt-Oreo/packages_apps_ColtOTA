...
package org.coltos.ota.receivers;
...
import org.coltos.ota.ColtOTA;
import org.coltos.ota.R;
import org.coltos.ota.RomUpdate;
import org.coltos.ota.activities.AddonActivity;
import org.coltos.ota.activities.AvailableActivity;
import org.coltos.ota.tasks.LoadUpdateManifest;
import org.coltos.ota.utils.Constants;
import org.coltos.ota.utils.Preferences;
import org.coltos.ota.utils.Utils;
...
public class AppReceiver extends BroadcastReceiver implements Constants{
	...
	public void onReceive(Context context, Intent intent) {
		...
		if (action.equals(DownloadManager.ACTION_DOWNLOAD_COMPLETE)) {
			...
			Set<Integer> set = ColtOTA.getAddonDownloadKeySet();
			Iterator<Integer> iterator = set.iterator();

			while (iterator.hasNext() && isAddonDownload != true) {
				int nextValue = iterator.next();
				if (id == ColtOTA.getAddonDownload(nextValue)) {
			...
			if (isAddonDownload) {
				...
				if (DownloadManager.STATUS_SUCCESSFUL != cursor.getInt(statusIndex)) {
					...
					ColtOTA.removeAddonDownload(keyForAddonDownload);
